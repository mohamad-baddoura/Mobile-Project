...
class AdvancedGame extends StatefulWidget {
  // A constructor that takes a key parameter
  const AdvancedGame({Key? key}) : super(key: key);
...
class _EmojiGameState extends State<AdvancedGame> {
  ...
  void initState() {
    ...
    emoji = result.split("\n")[0];
    text = EmojiPacks().emojiCombinationsMap[emoji]!;
  ...
  String getRandomEmojiAndChoices() {
    ...
    List<String> emojis = EmojiPacks().emojiCombinationsMap.keys.toList();
    // Create a list of all the values (text descriptions) in the Map
    List<String> texts = EmojiPacks().emojiCombinationsMap.values.toList();
    ...
    return Container(
        decoration: const BoxDecoration(
            image: DecorationImage(
                image: AssetImage('assets/emojis(travel & places).jpeg')
                ,fit: BoxFit.cover)
        ),
        child: Scaffold(
