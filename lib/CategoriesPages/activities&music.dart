...
class ActivitiesEmojiGame extends StatefulWidget {
  // A constructor that takes a key parameter
  const ActivitiesEmojiGame({Key? key}) : super(key: key);
...
class _EmojiGameState extends State<ActivitiesEmojiGame> {
  ...
  void initState() {
    ...
    emoji = result.split("\n")[0];
    text = EmojiPacks().activitiesMusicMap[emoji]!;
  ...
  String getRandomEmojiAndChoices() {
    ...
    List<String> emojis = EmojiPacks().activitiesMusicMap.keys.toList();
    // Create a list of all the values (text descriptions) in the Map
    List<String> texts = EmojiPacks().activitiesMusicMap.values.toList();
            ...
            actions: [
              ElevatedButton(
                onPressed: (){
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const ActivitiesEmojiGame()),
                  );
    ...
    return Container(
        decoration: const BoxDecoration(
            image: DecorationImage(
                image: AssetImage('assets/emojis(activities & music).jpeg')
                ,fit: BoxFit.cover)
        ),
        child: Scaffold(
