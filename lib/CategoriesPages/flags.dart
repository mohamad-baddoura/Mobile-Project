...
class FlagsEmojiGame extends StatefulWidget {
  // A constructor that takes a key parameter
  const FlagsEmojiGame({Key? key}) : super(key: key);
...
class _EmojiGameState extends State<FlagsEmojiGame> {
  ...
  void initState() {
    ...
    emoji = result.split("\n")[0];
    text = EmojiPacks().flagsMap[emoji]!;
  ...
  String getRandomEmojiAndChoices() {
    ...
    List<String> emojis = EmojiPacks().flagsMap.keys.toList();
    // Create a list of all the values (text descriptions) in the Map
    List<String> texts = EmojiPacks().flagsMap.values.toList();
            ...
            actions: [
              ElevatedButton(
                onPressed: (){
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const FlagsEmojiGame()),
                  );
    ...
    return Container(
        decoration: const BoxDecoration(
            image: DecorationImage(
                image: AssetImage('assets/emojis(flags).jpeg')
                ,fit: BoxFit.cover)
        ),
        child: Scaffold(
