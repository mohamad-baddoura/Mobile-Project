...
class FoodEmojiGame extends StatefulWidget {
  // A constructor that takes a key parameter
  const FoodEmojiGame({Key? key}) : super(key: key);
...
class _EmojiGameState extends State<FoodEmojiGame> {
  ...
  void initState() {
    ...
    emoji = result.split("\n")[0];
    text = EmojiPacks().foodDrinksMap[emoji]!;
  ...
  String getRandomEmojiAndChoices() {
    ...
    List<String> emojis = EmojiPacks().foodDrinksMap.keys.toList();
    // Create a list of all the values (text descriptions) in the Map
    List<String> texts = EmojiPacks().foodDrinksMap.values.toList();
            ...
            actions: [
              ElevatedButton(
                onPressed: (){
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const FoodEmojiGame()),
                  );
    ...
    return Container(
        decoration: const BoxDecoration(
            image: DecorationImage(
                image: AssetImage('assets/emojis(food & drinks).jpeg')
                ,fit: BoxFit.cover)
        ),
        child: Scaffold(
