...
class SmileysEmojiGame extends StatefulWidget {
  // A constructor that takes a key parameter
  const SmileysEmojiGame({Key? key}) : super(key: key);
...
class _EmojiGameState extends State<SmileysEmojiGame> {
  ...
  void initState() {
    ...
    emoji = result.split("\n")[0];
    text = EmojiPacks().smileysPeopleMap[emoji]!;
  ...
  String getRandomEmojiAndChoices() {
    ...
    List<String> emojis = EmojiPacks().smileysPeopleMap.keys.toList();
    // Create a list of all the values (text descriptions) in the Map
    List<String> texts = EmojiPacks().smileysPeopleMap.values.toList();
            ...
            actions: [
              ElevatedButton(
                onPressed: (){
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const SmileysEmojiGame()),
                  );
    ...
    return Container(
        decoration: const BoxDecoration(
            image: DecorationImage(
                image: AssetImage('assets/emojis(smileys and people).jpeg')
                ,fit: BoxFit.cover)
        ),
        child: Scaffold(
